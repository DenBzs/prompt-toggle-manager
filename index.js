...
function buildGroupCard(g, gi, pn) {
    ...
    const rows = g.toggles.map((t, ti) => {
        ...
        <div class="ptm-trow" ${inToggleReorder?'draggable="true"':''} data-gi="${gi}" data-ti="${ti}">
            ...
            <button class="ptm-ibtn ptm-bsel ${isDirect?'ptm-bsel-dir':'ptm-bsel-inv'}" data-gi="${gi}" data-ti="${ti}">${isDirect?'동일':'반전'}</button>
    ...
    <div class="ptm-card" data-gi="${gi}">
        ...
        ${!groupReorderMode && !inToggleReorder ? `<button class="ptm-sm ptm-sm-full ptm-add-toggle${isCollapsed?' ptm-hidden':''}" data-gi="${gi}">+ 토글 추가</button>` : ''}
    </div>`;
}
}
