...
let dragState = null; // { type:'group'|'toggle', gi, ti? }
...
function buildGroupCard(g, gi, pn) {
    ...
    const rows = g.toggles.map((t, ti) => {
        ...
        return `
        <div class="ptm-trow${reorderMode?' ptm-reorder-row':''}" ${reorderMode?'draggable="true"':''} data-gi="${gi}" data-ti="${ti}">
            ${reorderMode ? `<span class="ptm-drag-handle" title="드래그하여 이동">⠿</span>` : `<span class="ptm-tstate ${effectiveOn?'ptm-ts-on':'ptm-ts-off'}">${effectiveOn?'On':'Off'}</span>`}
            <button class="ptm-ibtn ptm-tovr ${ovrCls}" data-gi="${gi}" data-ti="${ti}">${ovrLabel}</button>
            <span class="ptm-tname">${name}</span>
            ${reorderMode ? '' : `<button class="ptm-ibtn ptm-bsel ${isDirect?'ptm-bsel-dir':'ptm-bsel-inv'}" data-gi="${gi}" data-ti="${ti}">${isDirect?'동일':'반전'}</button>`}
            ${reorderMode ? '' : `<button class="ptm-ibtn ptm-danger ptm-del-toggle" data-gi="${gi}" data-ti="${ti}">✕</button>`}
        </div>`;
    }).join('');
    ...
    return `
    <div class="ptm-card${reorderMode?' ptm-reorder-card':''}" ${reorderMode?'draggable="true"':''} data-gi="${gi}">
        <div class="ptm-card-head">
            ${reorderMode ? `<span class="ptm-drag-handle ptm-drag-handle-lg" title="드래그하여 이동">⠿</span>` : ''}
            ...
            <div class="ptm-gbtns">
                ${reorderMode ? '' : `<button class="ptm-ibtn ptm-ren-grp" data-gi="${gi}">✏️</button>`}
                ${reorderMode ? '' : `<button class="ptm-ibtn ptm-reorder-grp-btn" data-gi="${gi}" title="순서 변경">⠿</button>`}
                ${reorderMode ? '' : `<button class="ptm-ibtn ptm-danger ptm-del-grp" data-gi="${gi}">✕</button>`}
        ...
        ${reorderMode ? '' : `<button class="ptm-sm ptm-sm-full ptm-add-toggle${isCollapsed?' ptm-hidden':''}" data-gi="${gi}">+ 토글 추가</button>`}
    </div>`;
}
...
function wireGroupCards(area) {
    area.querySelectorAll('.ptm-reorder-grp-btn').forEach(btn => btn.addEventListener('click', () => {
        reorderMode = true;
        const globalBtn = document.getElementById('ptm-reorder-btn');
        const addBtn = document.getElementById('ptm-add-group');
        if (globalBtn) { globalBtn.textContent = '✓ 완료'; globalBtn.style.color = '#6ddb9e'; }
    ...
    <div class="inline-drawer">
        ...
        <div class="inline-drawer-content">
            ...
            <div style="display:flex;gap:6px;margin-top:0">
                ...
                <button class="ptm-sm" id="ptm-reorder-btn" style="margin:0;padding:3px 10px" title="순서 변경">⠿</button>
            </div>
        </div>
    </div>`;
    return el;
...
function wireMover() {
    // Refresh preset list every time the mover drawer is opened
    document.querySelector('#ptm-mover-drawer .inline-drawer-toggle')?.addEventListener('click', () => {
        // drawer opens on click — refresh after toggle animation (next tick)
        setTimeout(refreshPresetSelects, 0);
    });
...
function wireTG() {
    ...
    document.getElementById('ptm-reorder-btn')?.addEventListener('click', () => {
        ...
        const addBtn = document.getElementById('ptm-add-group');
        if (btn) { btn.textContent = reorderMode ? '✓ 완료' : '⠿'; btn.style.color = reorderMode ? '#6ddb9e' : ''; }
        if (addBtn) addBtn.style.display = reorderMode ? 'none' : '';
        renderTGGroups();
    });
    wireTGReorder();
}
...
function wireTGReorder() {
    ...
    area.addEventListener('dragstart', e => {
        ...
        const card = e.target.closest('.ptm-card[draggable]');
        if (row) {
            dragState = { type: 'toggle', gi: +row.dataset.gi, ti: +row.dataset.ti };
            e.dataTransfer.effectAllowed = 'move';
            setTimeout(() => row.classList.add('ptm-dragging'), 0);
        } else if (card) {
            dragState = { type: 'group', gi: +card.dataset.gi };
            e.dataTransfer.effectAllowed = 'move';
            setTimeout(() => card.classList.add('ptm-dragging'), 0);
        }
    });

    area.addEventListener('dragover', e => {
        if (!reorderMode || !dragState) return;
        e.preventDefault();
        e.dataTransfer.dropEffect = 'move';
        area.querySelectorAll('.ptm-drag-over').forEach(el => el.classList.remove('ptm-drag-over'));
        if (dragState.type === 'group') {
            const card = e.target.closest('.ptm-card[draggable]');
            if (card && +card.dataset.gi !== dragState.gi) card.classList.add('ptm-drag-over');
        } else if (dragState.type === 'toggle') {
            const row = e.target.closest('.ptm-trow[draggable]');
    ...
    area.addEventListener('drop', e => {
        ...
        if (dragState.type === 'group') {
            const card = e.target.closest('.ptm-card[draggable]');
            if (!card || +card.dataset.gi === dragState.gi) { dragState = null; return; }
            const [moved] = gs.splice(dragState.gi, 1);
            gs.splice(+card.dataset.gi, 0, moved);
            saveGroups(pn, gs); renderTGGroups();
        } else if (dragState.type === 'toggle') {
            const row = e.target.closest('.ptm-trow[draggable]');
