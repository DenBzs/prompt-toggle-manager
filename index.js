...
// ── Preset picker — uses callGenericPopup ────────────────────────
async function showPresetPicker(gi, isMove) {
    ...
    const html = `<div style="display:flex;flex-direction:column;gap:6px;padding:4px 0">
        ${others.map(n => `<button class="ptm-pick-preset menu_button" data-preset="${n}" style="text-align:left;padding:8px 12px">${n}</button>`).join('')}
    </div>`;

    const popup = await callGenericPopup(html, POPUP_TYPE.TEXT, '', { okButton: false, cancelButton: '닫기' });
    // callGenericPopup with TEXT just shows content; we wire click events on the buttons
    // But since popup closes on ok/cancel, we handle via delegated click before it closes
    // Alternative: wire before showing
    // Actually callGenericPopup shows HTML and we need to intercept button clicks inside it
    // The popup is rendered into a modal div by ST — we can query after showing
    document.querySelectorAll('.ptm-pick-preset').forEach(btn => {
        btn.addEventListener('click', () => {
            const dst = btn.dataset.preset, dg = getGroupsForPreset(dst);
            let name = g.name, c = 1;
            ...
            toastr.success(`"${name}" → "${dst}" ${isMove?'이동':'복사'} 완료`);
            // close ST popup
...
// ── Add toggle modal — uses callGenericPopup ─────────────────────
async function showAddToggleModal(gi) {
    ...
    const html = `
        <input type="text" id="ptm-msearch" placeholder="검색..." style="width:100%;margin-bottom:8px;padding:6px 8px;border-radius:5px;border:1px solid #555;background:#222;color:#eee;box-sizing:border-box">
        <div id="ptm-mlist" style="max-height:50vh;overflow-y:auto;display:flex;flex-direction:column;gap:2px">
            ${prompts.map(p => {
                const ex = exists.has(p.identifier);
                return `<label style="display:flex;align-items:center;gap:8px;padding:6px 4px;cursor:${ex?'default':'pointer'};opacity:${ex?'0.45':'1'}">
                    <input type="checkbox" data-id="${p.identifier}" ${ex?'disabled checked':''} style="width:16px;height:16px;accent-color:#7a6fff;flex-shrink:0">
                    <span>${p.name||p.identifier}</span>
                    ${ex?'<span style="font-size:10px;padding:1px 5px;border-radius:8px;background:rgba(120,100,255,.25);color:#a89fff">추가됨</span>':''}
                </label>`;
            }).join('')}
        </div>`;

    // wire search before popup opens
    // We must wire AFTER popup renders — use MutationObserver trick or just wire inside ok handler
    const ok = await callGenericPopup(html, POPUP_TYPE.CONFIRM, '', { okButton: '추가', cancelButton: '취소' });

    // wire search (runs after popup DOM is ready — this fires after user interaction, so wire immediately via timeout)
    setTimeout(() => {
    ...
    if (!ok) return;
    const sel = [...document.querySelectorAll('#ptm-mlist input:checked:not(:disabled)')].map(c => c.dataset.id);
    if (!sel.length) { toastr.warning('추가할 항목을 선택하세요'); return; }
    const gs2 = getGroupsForPreset(pn);
    sel.forEach(id => gs2[gi].toggles.push({ target: id, behavior: 'direct' }));
